...
def log(connection, target, action):
    ...
    query = text("INSERT INTO action_log (user_id, action_type, details, timestamp) VALUES (:user_id, :action, :details, :timestamp)")
    connection.execute(query, {"user_id": user_id, "action": action_type, "details": details, "timestamp": datetime.now()})
